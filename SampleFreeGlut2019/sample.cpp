...
// This is the shaders assignment
// Trying to work with shaders kin of sucks
//
//	Author:			Logan Kleditz

// title of these windows:
#define MSEC 125000 // i.e., 10 seconds
const char *WINDOWTITLE = { "OpenGL / Final Project -- Logan Kleditz" };
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->nz = z;
			if (Distort_on) {
				p->s = (cos(xz) * 1/Time) * ((lng + M_PI ) / (2.0 * M_PI));
				p->t =  ((lat + M_PI / 2.) / M_PI);
			}
			else {
				p->s = (lng + M_PI) / (2. * M_PI);
...
main( int argc, char *argv[ ] )
{
	...
	glutInit( &argc, argv );
	//0
	Xpos.AddTimeValue(0., .013);
	Zpos.AddTimeValue(0., 0.);
	ThetaY.AddTimeValue(0, 0.);
	ThetaX.AddTimeValue(0, -2.);
	//8
	Zpos.AddTimeValue(8., 2.);
	ThetaY.AddTimeValue(8, 65.);
	ThetaX.AddTimeValue(8, 15.);
	//15
	Zpos.AddTimeValue(15., 3.);
	//20
	Xpos.AddTimeValue(20., 0.);
	ThetaX.AddTimeValue(20, 15.);
	ThetaY.AddTimeValue(20, 65.);
	Zpos.AddTimeValue(20, 2.);
	//30
	ThetaY.AddTimeValue(30, 0.);
	ThetaX.AddTimeValue(30., -10.);
	//35
	ThetaX.AddTimeValue(35., 30.);
	ThetaY.AddTimeValue(35., -20.);
	//42
	ThetaX.AddTimeValue(42.5, 30.);
	ThetaY.AddTimeValue(42.5, -20.);
	Zpos.AddTimeValue(42.5, 2.);

	//52.5
	ThetaX.AddTimeValue(52.5, 10.);
	ThetaY.AddTimeValue(52.5, -35.);
	Zpos.AddTimeValue(52.5, 0.);
	
	//60
	Zpos.AddTimeValue(52.5, 0.65);

	//67.5
	ThetaX.AddTimeValue(67.5, 25.);
	ThetaY.AddTimeValue(67.5, -87.);
	Zpos.AddTimeValue(67.5, 0.);

	//75
	ThetaX.AddTimeValue(75, -90.);
	ThetaY.AddTimeValue(75, -87.);
	Zpos.AddTimeValue(75, 2.);
	//80
	Zpos.AddTimeValue(80, 0.);
	//95
	ThetaX.AddTimeValue(95, -90.);
	ThetaY.AddTimeValue(95, 180.);
	Zpos.AddTimeValue(95, 0.);

	//105
	ThetaX.AddTimeValue(110.5, 90.);
	ThetaY.AddTimeValue(110.5, 180.);
	Zpos.AddTimeValue(110.5, 0.);

	//125
	ThetaX.AddTimeValue(125, 0.);
	ThetaY.AddTimeValue(125, 0.);
	Zpos.AddTimeValue(125, 0.);
...
Display( )
{
	...
	//gluLookAt( 0., 0., 15 - (Time * 3),     0., 0., 0.,     0., 1., 0. );
	...
	float lightx = 20., lighty = 20., lightz = 20.;
	float uKa = 0.75, uKd = 0.5, uKs = 0.65;
	float uScale = 0.1225;
	float shine = 2, uSize = 2.0;
	float ColorR = .5, ColorG = .5, ColorB = .5;
	/*glPushMatrix();
	SetSpotLight(GL_LIGHT1, 0, 0., 15., 0, 0, -1, 1., 1., 1.);
	glPopMatrix();
	if (Light0On) {
		glEnable(GL_LIGHT1);
	}
	...
	/*Texture = BmpToTexture("lroc_color_poles_2k.bmp", &width, &height);
	Texture2 = BmpToTexture("ldem_4.bmp", &nums, &numt);*/
	...
	Pattern->SetUniformVariable("uDispUnit", 2);
	Pattern->SetUniformVariable("uScale", uScale);
	...
	Pattern->SetUniformVariable("LightPosition", 0, 0., 15.); 
	//Pattern->SetUniformVariable("uColor", uColor)
...
{
	Distort_on = (id == Distort);
	tex_on = (id != No_texture);
	glutSetWindow(MainWindow);
	glutPostRedisplay();
}
...
InitMenus( )
{
	...
	glutAddMenuEntry("No Tex", No_texture);
	glutAddMenuEntry("Texture", Tex_nDist);
...
InitGraphics( )
{
	...
	int width, height, nums, numt;
	Texture = BmpToTexture("lroc_color_poles_8k.bmp", &width, &height); //lroc_color_poles_2k.bmp
	Texture2 = BmpToTexture("ldem_16.bmp", &nums, &numt); //ldem_4.bmp
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, nums, numt, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture2);
